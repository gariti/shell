...
Singleton {
    ...
    Process {
        ...
        running: true
        command: ["sh", "-c", "ls /sys/class/power_supply/BAT* 2>/dev/null | head -1 | xargs -I {} sh -c 'if [ -f {}/capacity ] && [ -f {}/status ]; then echo \"$(cat {}/capacity) $(cat {}/status)\"; else echo \"0 Unknown\"; fi'"]
        stdout: SplitParser {
            onRead: data => {
                const parts = data.trim().split(" ");
                if (parts.length >= 2) {
                    const capacity = parseInt(parts[0], 10);
                    const status = parts[1];
                    
                    root.hasBattery = capacity > 0 || status !== "Unknown";
                    root.batteryLevel = capacity;
                    root.isCharging = status.includes("Charging");
                } else {
